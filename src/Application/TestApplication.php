...
namespace Weble\JoomlaTestBench\Application;
...
class TestApplication extends CMSApplication
{
     ...
     * @since   3.2
     */
    public function __construct(TestInput $input = null, Registry $config = null, \JApplicationWebClient $client = null)
    {
    ...
    public function setInput(TestInput $input): self
    {
        $this->input = $input;

        foreach ($this->afterInputSet as $callback) {
    ...
    public function afterInputSet(callable $callback): void
    {
        $this->afterInputSet[] = $callback;
    }

    /**
     * Check if the user can access the application
     *
     * @param int $itemid The item ID to check authorisation for
     *
     * @return  void
     *
    ...
    protected function authorise($itemid)
    {
        ...
        if (! $menus->authorise($itemid)) {
            if ($user->get('id') == 0) {
                // Set the data
                $this->setUserState('users.login.form.data', ['return' => \JUri::getInstance()->toString()]);
     ...
     *
     * @param bool $params True to return the template parameters
     *
     ...
     *
     * @param bool $state The new state of the detect browser option
     *
     ...
     *
     * @param bool $state The new state of the language filter
     *
     * @return    bool     The previous state
     *
     * @since    3.2
    ...
    {
        $response = new TestResponse($this->response);
        $this->dispatcher->trigger('onTestFinished', ['response' => $response]);
